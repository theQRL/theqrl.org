...
function get_contributor_urls($org) {
	...
	$contributors = json_decode(file_get_contents("https://api.github.com/orgs/".$org."/repos", false, $context));
	$contributor_urls = [];

	foreach ($contributors as $key => $value) {
		if($value->fork==false) {
			$contributor_urls[] = $value->contributors_url;
		}
	}
...
yaml_emit_file("_data/contributions.yml", merge_contributor_data($contributor_data));